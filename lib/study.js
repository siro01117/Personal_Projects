...
import { normLecture } from './lecture.mjs';
import { normNote } from './stages.mjs';
...
function normLesson(s, i) {
  ...
  return {
    ...
    // summary/keyPoints/needsCheck/concepts 는 정리해서 넣어주는 값 — 화면에서 읽기 전용.
    summary: str(pick(o, 'summary', 'digest'), ''),
    keyPoints: arr(pick(o, 'keyPoints', 'key_points', 'highlights')).map((x) => str(x)).filter(Boolean),
    // 지금 단계(학기 초)에 먼저 오는 것 — 과목 특유의 사고방식과 명명 체계.
    // 시험 대비 인출은 뒤로 접어둔다. 순서가 곧 학습 단계다.
    frame: str(pick(o, 'frame', 'lens', 'mindset'), ''),
    // 학기 초에 실제로 필요한 네 가지. 길게 쓰지 않는다.
    flow: arr(pick(o, 'flow', 'thinking')).map((x) => str(x)).filter(Boolean),
    context: arr(pick(o, 'context', 'background')).map((x) => str(x)).filter(Boolean),
    principles: arr(pick(o, 'principles', 'rules')).map((x) => str(x)).filter(Boolean),
    keyTerms: arr(pick(o, 'keyTerms', 'coreTerms')).map((t, j) => ({
      id: str(t?.id, `kt${j}`),
      term: str(pick(t || {}, 'term', 'name')),
      def: str(pick(t || {}, 'def', 'line', 'meaning')),
    })).filter((t) => t.term),
    naming: arr(pick(o, 'naming', 'terms')).map((n, j) => ({
      id: str(n?.id, `nm${j}`),
      term: str(pick(n || {}, 'term', 'name')),
      why: str(pick(n || {}, 'why', 'reason', 'origin')),
    })).filter((n) => n.term),
    // 인출(retrieval) 중심 — 답을 먼저 주지 않는다. 질문을 주고 눌러야 답이 나온다.
    retrieval: arr(pick(o, 'retrieval', 'questions')).map((q, j) => ({
      id: str(q?.id, `q${j}`),
      q: str(pick(q || {}, 'q', 'question', 'front')),
      a: str(pick(q || {}, 'a', 'answer', 'back')),
    })).filter((q) => q.q),
    // 명명·압축 — 꺼내 쓸 수 있는 한 줄짜리 이름표
    compress: arr(pick(o, 'compress', 'naming')).map((c, j) => ({
      id: str(c?.id, `c${j}`),
      term: str(pick(c || {}, 'term', 'name', 'title')),
      line: str(pick(c || {}, 'line', 'oneLine', 'def')),
    })).filter((c) => c.term),
    // 비교·대조 — 본질은 차이에서 드러난다
    contrast: arr(pick(o, 'contrast', 'compare')).map((c, j) => ({
      id: str(c?.id, `x${j}`),
      pair: str(pick(c || {}, 'pair', 'title')),
      axis: str(pick(c || {}, 'axis', 'basis')),
      diff: str(pick(c || {}, 'diff', 'detail', 'body')),
    })).filter((c) => c.pair),
    needsCheck: arr(pick(o, 'needsCheck', 'needs_check', 'unclear')).map((x) => str(x)).filter(Boolean),
    concepts: arr(pick(o, 'concepts', 'terms')).map((x) => str(x)).filter(Boolean),
    // 수업 중 든 의문 — 사용자가 그 자리에서 적는다. 나중에 복기 질문이 된다.
    // 자기가 만든 질문이 남이 만든 질문보다 인출 자극이 세다.
    asks: arr(pick(o, 'asks', 'myQuestions')).map((q, j) => ({
      id: str(q?.id, `ask${j}`),
      q: str(pick(q || {}, 'q', 'text', 'question')),
      a: str(pick(q || {}, 'a', 'answer'), ''),
      at: str(pick(q || {}, 'at', 'time'), ''),
    })).filter((q) => q.q),
    // note 는 사용자가 직접 쓰는 칸. 정리 결과와 절대 섞지 않는다.
    note: str(pick(o, 'note'), ''),
    ink: arr(o.ink),
  };
}
...
function normCourse(c, i) {
  ...
  return {
    ...
    // 단원 강의 — 교재 한 단원을 강의 형식으로 쓴 글. 구조는 lib/lecture.mjs 가 정한다.
    units: arr(pick(o, 'units', 'lectures')).map(normLecture),
    // 내가 스테이지로 채워 넣는 정리. 사이트에서 쓰는 유일한 입력이다.
    notes: arr(o.notes).map(normNote),
