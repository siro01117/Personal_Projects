  ...
  return entries
    ...
    .map((e) => {
      ...
      return {
        ...
        color: typeof meta.color === 'string' ? meta.color : null,   // 모듈 테마색 (없으면 팔레트 자동배정)
        icon: typeof meta.icon === 'string' ? meta.icon : null,      // 아이콘 키워드
        section: typeof meta.section === 'string' ? meta.section : null,  // Personal | Display | Study

        // 정적추출은 디렉터리를 클린 URL(/projects/<id>)로 서빙 — index.html 경로는 404가 됨
        embedUrl: meta.embedUrl || (hasIndex ? `/projects/${e.name}/` : ''),
      };
    })
