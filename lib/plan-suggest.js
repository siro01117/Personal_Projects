...
export function travelBlocks(occurrences, dateISO, settings) {
  ...
  for (const o of timed) {
    ...
    if (home && at && at !== home && to && to !== home && lastKnownEnd != null) {
      ...
      if (back > 0 && again > 0 && o.start - lastKnownEnd - (back + prep + again) >= HOME_STAY_MIN) {
        out.push([lastKnownEnd, lastKnownEnd + back, '귀가']);
        at = home;
      }
    }
...
export function checkSlot({ occurrences, date, start, duration, settings, target, todayISO, nowMin }) {
  ...
  if (travelBlocks([...day, probe], date, settings).some(([a, b]) => a < cand.end && cand.start < b)) {
    return { ok: false, why: '오가는 길·준비 시간과 겹침' };
  }
  const ctx = {
  ...
  return { ok: true, score: r.score, reasons: r.reasons };
}
