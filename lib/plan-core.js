...
function normTask(t, i) {
  ...
  return {
    ...
    // 장소가 정해진 할 일 — 제안할 때 앞뒤 동선을 같이 계산한다(plan-suggest routeFit)
    place: str(o.place, ''),
    placeId: str(o.placeId, ''),
    travelMin: o.travelMin == null ? null : Math.max(0, num(o.travelMin, 0)),
    note: str(o.note, ''),
...
export function normalize(raw) {
  ...
  return {
    ...
    settings: {
      ...
      workPlaceId: str(s.workPlaceId, ''), // 스큐 근무가 열리는 지점 — 여기로 가는 이동이 '출근'
      schoolPlaceId: str(s.schoolPlaceId, ''), // 대학 수업이 열리는 지점
      prepMin: Math.max(0, num(s.prepMin, 0)), // 집에서 나가기 전 준비 시간(분)
      mealMin: Math.max(0, num(s.mealMin, 40)), // 식사 추천 길이(분). 0 이면 추천 안 함
      ...
      for (const m of c.meetings || []) {
        ...
        const list = expandWithExceptions(occursOnFn, overrides, fromISO, toISO, (origDate, ex, moved) => {
          ...
          const occ = makeOcc({
            ...
            placeId: doc.settings.schoolPlaceId || '',
  ...
  if (doc.settings.showWork) {
    for (const s of arr(shifts)) {
      ...
      out.push(makeOcc({
        ...
        placeId: doc.settings.workPlaceId || '',
  ...
  for (const t of doc.tasks) {
    ...
    if (t.slot.date >= fromISO && t.slot.date <= toISO) {
      ...
      out.push(makeOcc({
        ...
        allDay: false, important: false, color: 0, place: t.place || '',
        placeId: t.placeId || '', travelMin: t.travelMin,
...
/* ------------------------------------------------------- 저장 시 합치기 */
// 탭 하나가 기억하던 문서를 통째로 덮어쓰면, 그 사이 다른 곳(다른 탭·기기·서버 작업)에서
// 넣은 일정이 사라진다 — 실제로 열어둔 탭이 할 일 하나를 저장하면서 밖에서 넣은 일정 8개를 지웠다.
// 그래서 저장은 3자 합치기로 한다:
//   base   = 이 탭이 마지막으로 서버와 맞춘 문서
//   local  = 이 탭의 지금 문서
//   server = 저장 직전에 다시 읽은 서버 문서
// 이 탭이 바꾼 것(local ≠ base)만 server 위에 얹는다. 일정·할 일은 id 단위, 설정·수업 변경은 키 단위.
// base 를 모르면(옛 대기열 등) 지우기는 하지 않고 추가·수정만 얹는다 — 모르고 지우는 게 제일 나쁘다.
const same = (a, b) => JSON.stringify(a) === JSON.stringify(b);

function mergeList(base, local, server) {
  const byId = (xs) => new Map(arr(xs).filter((x) => x && x.id).map((x) => [x.id, x]));
  const b = base ? byId(base) : null;
  const l = byId(local);
  const out = new Map(byId(server));
  for (const [id, item] of l) {
    if (b && b.has(id) && same(b.get(id), item)) continue; // 이 탭에선 안 건드림 → 서버 것 유지
    out.set(id, item);                                      // 이 탭이 추가·수정
  }
  if (b) {
    for (const id of b.keys()) if (!l.has(id)) out.delete(id); // 이 탭이 지움
  }
  return [...out.values()];
}
...
function mergeObject(base, local, server) {
  ...
  for (const [k, v] of Object.entries(local || {})) {
    if (b && k in b && same(b[k], v)) continue;
    out[k] = v;
  }
  if (b) {
    for (const k of Object.keys(b)) if (!(k in (local || {}))) delete out[k];
  }
  return out;
}

export function mergePlan(base, local, server) {
  if (!server) return local;
  const merged = mergeObject(base, local, server); // 최상위(version 등)
  merged.settings = mergeObject(base?.settings, local?.settings, server?.settings);
  merged.classOverrides = mergeObject(base?.classOverrides, local?.classOverrides, server?.classOverrides);
  merged.events = mergeList(base?.events, local?.events, server?.events);
  merged.tasks = mergeList(base?.tasks, local?.tasks, server?.tasks);
