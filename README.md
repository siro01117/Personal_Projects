...
- `out/open.html`(페이지) 과 `out/open/`(폴더) 이 같은 이름이면 하위 경로가 404 가 된다. 그래서 시안은 `public/demos/` 에 둔다.
- **Vercel 은 cleanUrls 로 `.html` 을 떼고 서빙한다.** `DEMOS` 의 `file` 에는 확장자를 빼고 적는다. `/demos/x.html` 로 요청하면 404 가 난다.
